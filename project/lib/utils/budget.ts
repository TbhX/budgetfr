import { Budget, Mission, Revenue, EconomicIndicators } from "@/types/budget"
import { MISSION_CATEGORIES, REVENUE_CATEGORIES, DEFAULT_ECONOMIC_INDICATORS } from "@/lib/config/constants"
...
export function getInitialBudget(year: number): Budget {
  const missions = MISSION_CATEGORIES.map(category => ({
    id: category.id,
    name: category.name,
  ...
  const debtRatio = calculateDebtRatio(DEFAULT_ECONOMIC_INDICATORS.publicDebt, DEFAULT_ECONOMIC_INDICATORS.gdp)
